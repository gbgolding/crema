...
test_info = np.array([[test_sub[gene][feature] for feature in sorted(test_sub[gene])] for gene in subset_names], dtype=float) 

test_info_n = preprocessing.normalize(test_info, norm='l2') 
...
lncRNA_score = test_prob[:,1] # slicing array to only include this one score
lncRNA_predict = clf.predict(test_info_n)
